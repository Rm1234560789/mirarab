...
import {
    deleteObida,
    fetchCurrentObida, fetchCurrentObidaSuccess,
    getObida,
    getObidaSucces,
    saveObida,
    updateObida
} from "../reducer/obidaReducer.js";
...
export function* workFetchCurrentObida(action) {
    try {
        const res = yield call(() => axios({
            url: `http://localhost:8080/obida/${action.payload}`,
            method: "GET"
        }));
        yield put(fetchCurrentObidaSuccess(res.data));
    } catch (e) {
        alert("some error");
    }
}
...
export function* obidaSaga() {
    ...
    yield takeLatest(fetchCurrentObida().type, workFetchCurrentObida);
}
