...
import { getVideoSucces, getVideo, saveVd, deleteVd, editVd } from "../reducer/videoReducer.js";

export function* workGetVd() {
    try {
        const res = yield call(() => axios.get("http://localhost:8080/vd"));
...
export function* workPostVd(action) {
    try {
        ...
        data.url = res.data;
         yield call(() => axios.post("http://localhost:8080/vd", data));
        yield put(getVideo());
    } catch (e) {
        alert("Some error");
    }
...
export function* workDeleteVd(action) {
    try {
        yield call(() => axios.delete(`http://localhost:8080/vd/${action.payload}`));
        yield put(getVideo());
    } catch (e) {
        alert("Some error");
    }
...
export function* workEditVd(action) {
    ...
    try {
        ...
        yield call(() => axios.put(`http://localhost:8080/vd/${action.payload.id}`, {name:data.name,url:data.url}));
        yield put(getVideo());
        yield put(setImg(""));
    } catch (e) {
        alert("Some error");
