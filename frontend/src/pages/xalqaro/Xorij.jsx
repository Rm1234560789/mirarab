...
import {deleteXorij, getXorij, saveXorij, updateXorij} from "../../reducer/xorijReducer.js";

function Islom() {
    const {register, reset, handleSubmit, setValue} = useForm();
    const dispatch = useDispatch();
    const {xorij} = useSelector((state) => state.xorijReducer);
    ...
    function mySubmit(data) {
        if (isEdit !== '') {
            ...
            dispatch(updateXorij(data));
        } else {
            data.img = imgInp;
            dispatch(saveXorij(data));
        }
        reset();
        setIsEdit('');
        setCurrentId(null);
    ...
    return (
        <div className={'wrapper'}>
            <h1 className={"text"}>Xorijiy Professorlar</h1>
