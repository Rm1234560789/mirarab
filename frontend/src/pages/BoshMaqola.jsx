...
import {getMaqola,setDisplayImg, setImg} from '../reducer/newsReducer.js';
import {deleteMaqola, saveMaqola, updateMaqola} from "../reducer/maqolaReducer.js";

function BoshMaqola() {
    const {register, reset, handleSubmit, setValue} = useForm();
    const dispatch = useDispatch();
    const {maqola} = useSelector((state) => state.newsReducer);
    ...
    function mySubmit(data) {
        if (isEdit !== '') {
            ...
            dispatch(updateMaqola(data));
        } else {
            data.img = imgInp;
            dispatch(saveMaqola(data));
        }
        reset();
        setIsEdit('');
        setCurrentId(null);
    ...
    return (
        <div className={'wrapper'}>
            <h1 className={"text"}>Maqolalar</h1>
