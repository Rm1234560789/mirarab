...
import {getMadrasa, setDisplayImg, setImg} from '../reducer/newsReducer.js';
import { useNavigate } from 'react-router-dom';
import {deleteMadrasa, saveMadrasa, updateMadrasa} from "../reducer/madrasaHqReducer.js";

function Rahbariyat() {
    const { register, reset, handleSubmit, setValue } = useForm();
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { madrasa } = useSelector((state) => state.newsReducer);
    ...
    function mySubmit(data) {
        if (isEdit !== '') {
            ...
            dispatch(updateMadrasa(data));
        } else {
            data.img = imgInp;
            dispatch(saveMadrasa(data));
        }
        reset();
        setIsEdit('');
        setCurrentId(null);
    ...
    function handleReadMore(item) {
        navigate(`/madrasa/${item.id}`);
    }

    return (
        <div className={'wrapper'}>
            <h1 id="specificGroupArea" className={"text"}>Madrasa Haqida</h1>
            ...
            <table className={'table table-bordered'}>
                ...
                <tbody>
                {madrasa.map((item, index) => (
                    <tr key={index}>
                        <td>{item.title}</td>
                        <td>
                            <div style={{ maxHeight: '50px',overflow:"hidden",wordBreak:"break-all"}}>
                               {item.description}
